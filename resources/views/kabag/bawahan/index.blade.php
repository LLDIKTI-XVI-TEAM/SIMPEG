                    ...
                    <x-ui.table-body class="divide-y divide-border">
                        @forelse ($employees as $employee)
                            ...
                            <x-ui.table-row class="hover:bg-soft transition-colors group">
                                ...
                                <x-ui.table-td padding="comfortable">
                                    <div class="flex items-center gap-3">
                                        ...
                                        <div class="min-w-0">
                                            <x-ui.tooltip text="Buka detail {{ $employee->nama_lengkap }}" position="right">
                                                <a href="{{ route('kepala-bagian.bawahan.show', $employee) }}"
                                                    class="block truncate text-sm font-semibold text-ink transition-colors hover:text-primary focus:outline-none rounded leading-tight">{{ $employee->nama_lengkap }}</a>
                                            </x-ui.tooltip>
                                ...
                                <x-ui.table-td padding="comfortable">
                                    <x-ui.badge :variant="$employee->status_tampilan === 'Cuti' ? 'warning' : 'success'" size="sm" dot>
            ...
            <div class="flex flex-col items-start justify-between gap-4 border-t border-border bg-surface px-6 py-4 sm:flex-row sm:items-center">
                <div class="flex flex-wrap items-center gap-4 text-sm text-muted">
                    ...
                    <p class="whitespace-nowrap">
                        Menampilkan {{ $employees->firstItem() ?? 0 }} - {{ $employees->lastItem() ?? 0 }} dari {{ $employees->total() }}
                    </p>
                </div>
