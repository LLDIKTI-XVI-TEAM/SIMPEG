<x-layouts.app title="Profil Saya">
    ...
    @else
        <div x-data="{
            ...
            {{-- EWS WARNING SECTION --}}
            @if($ewsTotal > 0)
            <x-ui.alert variant="warning" title="Peringatan penting (EWS)">
                <div class="min-w-0">
                    <div class="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                        @if(auth()->user()?->role === 'pegawai')
                            <a href="{{ route('ews.saya') }}" class="text-xs font-semibold text-warning-dark hover:underline">Lihat semua</a>
                        @endif
                    </div>
                    <ul class="mt-1 space-y-1 text-xs text-warning-dark">
            ...
            <div class="rounded-lg border border-border bg-surface p-6 shadow-sm space-y-6">
                ...
                <div class="border-b border-border pb-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                    <div class="flex items-center gap-4">
                        ...
                        <div class="min-w-0">
                            <h1 class="text-2xl font-semibold text-ink font-sans leading-tight">{{ $p->nama_lengkap }}</h1>
                            <p class="text-xs text-muted">NIP. {{ $p->nip }} &bull; Email: {{ auth()->user()->email }}</p>
                            <div class="mt-1.5 flex flex-wrap items-center gap-2">
                                <x-ui.badge variant="primary" size="sm" pill uppercase>{{ $p->jenisPegawai->nama ?? '-' }}</x-ui.badge>
                                <x-ui.badge variant="muted" size="sm" pill>Role: {{ auth()->user()->getEffectiveRole() ?? 'Pegawai' }}</x-ui.badge>
                                @if(auth()->user()->keycloak_id)
                                    <x-ui.badge variant="success" size="sm" pill>SSO Terhubung</x-ui.badge>
                                @else
                                    <x-ui.badge variant="warning" size="sm" pill>SSO Belum Terhubung</x-ui.badge>
                                @endif
                                @if($p->is_kinerja_baik)
                                    <x-ui.badge variant="success" size="sm" pill>Kinerja Baik</x-ui.badge>
                                @endif
                            </div>
                        </div>
                    </div>
                ...
                {{-- TAB NAVIGATION --}}
                <x-ui.tabs label="Riwayat kepegawaian">
                    <x-ui.tab id="profile-tab" aria-controls="profile-panel" click="activeTab = 'profile'" active="activeTab === 'profile'">Profil</x-ui.tab>
                    <x-ui.tab id="cuti-tab" aria-controls="cuti-panel" click="activeTab = 'cuti'" active="activeTab === 'cuti'">Cuti</x-ui.tab>
                    <x-ui.tab id="keluarga-tab" aria-controls="keluarga-panel" click="activeTab = 'keluarga'" active="activeTab === 'keluarga'">Keluarga</x-ui.tab>
                    <x-ui.tab id="kepangkatan-tab" aria-controls="kepangkatan-panel" click="activeTab = 'kepangkatan'" active="activeTab === 'kepangkatan'">Kepangkatan</x-ui.tab>
                    <x-ui.tab id="jabatan-tab" aria-controls="jabatan-panel" click="activeTab = 'jabatan'" active="activeTab === 'jabatan'">Jabatan</x-ui.tab>
                    <x-ui.tab id="kgb-tab" aria-controls="kgb-panel" click="activeTab = 'kgb'" active="activeTab === 'kgb'">KGB</x-ui.tab>
                    <x-ui.tab id="disiplin-tab" aria-controls="disiplin-panel" click="activeTab = 'disiplin'" active="activeTab === 'disiplin'">Hukuman Disiplin</x-ui.tab>
                    <x-ui.tab id="pendidikan-tab" aria-controls="pendidikan-panel" click="activeTab = 'pendidikan'" active="activeTab === 'pendidikan'">Pendidikan</x-ui.tab>
                    <x-ui.tab id="pengangkatan-tab" aria-controls="pengangkatan-panel" click="activeTab = 'pengangkatan'" active="activeTab === 'pengangkatan'">Pengangkatan</x-ui.tab>
                    <x-ui.tab id="docs-tab" aria-controls="docs-panel" click="activeTab = 'docs'" active="activeTab === 'docs'">Dokumen SK</x-ui.tab>
                </x-ui.tabs>

                {{-- TAB 1: PROFIL LENGKAP --}}
                <div id="profile-panel" role="tabpanel" aria-labelledby="profile-tab" x-show="activeTab === 'profile'" class="space-y-6" x-transition>
                    ...
                    <div class="bg-soft/40 rounded-lg p-4 border border-border">
                        <div class="flex items-center gap-3">
                            ...
                            <div>
                                <span class="text-xs font-bold text-muted uppercase tracking-wider font-sans block">Atasan Langsung Anda</span>
                    ...
                    <div class="space-y-3">
                        <h3 class="text-sm font-semibold text-ink uppercase tracking-wider font-sans border-b border-border pb-1.5 flex items-center gap-1.5">
                            Estimasi Jadwal Kepegawaian
                        </h3>
                        <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                            <div class="rounded-lg border border-border bg-surface p-3 shadow-sm text-center">
                                <span class="text-xs font-bold text-muted uppercase tracking-wider font-sans">Kenaikan Pangkat Terdekat</span>
                                <p class="text-sm font-bold text-ink font-sans mt-1">{{ $estimasiPangkatNext }}</p>
                                <p class="text-xs text-muted font-sans mt-0.5">(Estimasi 4 tahun sejak TMT)</p>
                            </div>
                            <div class="rounded-lg border border-border bg-surface p-3 shadow-sm text-center">
                                <span class="text-xs font-bold text-muted uppercase tracking-wider font-sans">KGB Terdekat</span>
                                <p class="text-sm font-bold text-ink font-sans mt-1">{{ $estimasiKgbNext }}</p>
                                <p class="text-xs text-muted font-sans mt-0.5">(Estimasi 2 tahun sejak TMT)</p>
                            </div>
                            <div class="rounded-lg border border-border bg-surface p-3 shadow-sm text-center">
                                <span class="text-xs font-bold text-muted uppercase tracking-wider font-sans">Estimasi Tanggal Pensiun</span>
                                <p class="text-sm font-bold text-ink font-sans mt-1">{{ $estimasiPensiun }}</p>
                                <p class="text-xs text-danger font-semibold mt-0.5">Sisa: {{ $sisaPensiunStr }}</p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div class="space-y-4">
                            <h3 class="text-sm font-semibold text-ink uppercase tracking-wider font-sans border-b border-border pb-1.5">Identitas & Data Pribadi</h3>
                        ...
                        <div class="space-y-4">
                            <h3 class="text-sm font-semibold text-ink uppercase tracking-wider font-sans border-b border-border pb-1.5">Kontak & Rumah</h3>
                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 text-xs">
                                <div class="space-y-0.5">
                    ...
                    <div class="space-y-4 border-t border-border pt-4">
                        <h3 class="text-sm font-semibold text-ink uppercase tracking-wider font-sans border-b border-border pb-1.5">Informasi Pekerjaan Utama</h3>
                ...
                {{-- TAB X: INFO CUTI --}}
                <div id="cuti-panel" role="tabpanel" aria-labelledby="cuti-tab" x-show="activeTab === 'cuti'" style="display: none;" class="space-y-4" x-transition>
                ...
                {{-- TAB 2: DATA KELUARGA --}}
                <div id="keluarga-panel" role="tabpanel" aria-labelledby="keluarga-tab" x-show="activeTab === 'keluarga'" class="space-y-4" style="display: none;" x-transition>
                    <div class="flex items-center justify-between">
                        <div>
                                ...
                                <template x-if="keluargaList.length === 0">
                                    <x-ui.table-row>
                                        <x-ui.table-td colspan="5" class="px-0 py-0">
                                            <x-ui.empty-state icon="none" title="Tidak ada data anggota keluarga." />
                                        </x-ui.table-td>
                                    </x-ui.table-row>
                                </template>
                                <template x-for="(fam, index) in keluargaList" :key="index">
                                    <x-ui.table-row>
                                        <x-ui.table-td>
                                            ...
                                            <p class="text-xs text-muted" x-text="fam.nik ? 'NIK. ' + fam.nik : 'NIK. -'"></p>
                                        </x-ui.table-td>
                                        <x-ui.table-td>
                                            <p class="font-sans" x-text="fam.hubungan"></p>
                                            <p class="text-xs text-muted font-sans" x-text="fam.jenis_kelamin"></p>
                                        </x-ui.table-td>
                                        <x-ui.table-td>
                                            <p class="font-sans" x-text="fam.tempat_lahir || '-'"></p>
                                            <p class="text-xs text-muted" x-text="fam.tanggal_lahir"></p>
                                        </x-ui.table-td>
                ...
                <div id="kepangkatan-panel" role="tabpanel" aria-labelledby="kepangkatan-tab" x-show="activeTab === 'kepangkatan'" class="space-y-4" style="display: none;" x-transition>
                    <div>
                                ...
                                @empty
                                    <x-ui.table-row>
                                        <x-ui.table-td colspan="4" class="px-0 py-0">
                                            <x-ui.empty-state icon="none" title="Tidak ada data riwayat kepangkatan." />
                                        </x-ui.table-td>
                                    </x-ui.table-row>
                                @endforelse
                            </x-ui.table-body>
                ...
                <div id="jabatan-panel" role="tabpanel" aria-labelledby="jabatan-tab" x-show="activeTab === 'jabatan'" class="space-y-4" style="display: none;" x-transition>
                    <div>
                        ...
                        <x-ui.table>
                            ...
                            <x-ui.table-body>
                                ...
                                @empty
                                    <x-ui.table-row>
                                        <x-ui.table-td colspan="5" class="px-0 py-0">
                                            <x-ui.empty-state icon="none" title="Tidak ada data riwayat jabatan." />
                                        </x-ui.table-td>
                                    </x-ui.table-row>
                                @endforelse
                            </x-ui.table-body>
                ...
                {{-- TAB 5: RIWAYAT KGB --}}
                <div id="kgb-panel" role="tabpanel" aria-labelledby="kgb-tab" x-show="activeTab === 'kgb'" class="space-y-4" style="display: none;" x-transition>
                    <div>
                        ...
                        <x-ui.table>
                            ...
                            <x-ui.table-body>
                                ...
                                @empty
                                    <x-ui.table-row>
                                        <x-ui.table-td colspan="4" class="px-0 py-0">
                                            <x-ui.empty-state icon="none" title="Tidak ada data riwayat KGB." />
                                        </x-ui.table-td>
                                    </x-ui.table-row>
                                @endforelse
                            </x-ui.table-body>
                ...
                <div id="disiplin-panel" role="tabpanel" aria-labelledby="disiplin-tab" x-show="activeTab === 'disiplin'" class="space-y-4" style="display: none;" x-transition>
                    <div>
                        ...
                        <x-ui.table>
                            ...
                            <x-ui.table-body>
                                ...
                                @empty
                                    <x-ui.table-row>
                                        <x-ui.table-td colspan="5" class="px-0 py-0">
                                            <x-ui.empty-state icon="none" title="Anda tidak memiliki riwayat hukuman disiplin." />
                                        </x-ui.table-td>
                                    </x-ui.table-row>
                                @endforelse
                            </x-ui.table-body>
                ...
                <div id="pendidikan-panel" role="tabpanel" aria-labelledby="pendidikan-tab" x-show="activeTab === 'pendidikan'" class="space-y-4" style="display: none;" x-transition>
                    <div>
                        ...
                        <x-ui.table>
                            ...
                            <x-ui.table-body>
                                ...
                                @empty
                                    <x-ui.table-row>
                                        <x-ui.table-td colspan="5" class="px-0 py-0">
                                            <x-ui.empty-state icon="none" title="Tidak ada data riwayat pendidikan." />
                                        </x-ui.table-td>
                                    </x-ui.table-row>
                                @endforelse
                            </x-ui.table-body>
                ...
                <div id="pengangkatan-panel" role="tabpanel" aria-labelledby="pengangkatan-tab" x-show="activeTab === 'pengangkatan'" class="space-y-4" style="display: none;" x-transition>
                    <div>
                    ...
                    <div class="rounded-lg border border-border bg-surface p-6">
                        <x-ui.empty-state icon="none" title="Tidak ada data pengangkatan." />
                    </div>
                    @endif
                </div>

                {{-- TAB 9: BERKAS DOKUMEN SK --}}
                <div id="docs-panel" role="tabpanel" aria-labelledby="docs-tab" x-show="activeTab === 'docs'" style="display: none;" class="space-y-4" x-transition>
                    <div>
                    ...
                    <div class="overflow-x-auto">
                        <x-ui.table>
                            ...
                            <x-ui.table-body>
                                ...
                                <x-ui.table-row>
                                    <x-ui.table-td colspan="5" class="px-0 py-0">
                                        <x-ui.empty-state icon="none" title="Belum ada dokumen atau SK kepegawaian yang diunggah." />
                                    </x-ui.table-td>
                                </x-ui.table-row>
                                @endforelse
                            </x-ui.table-body>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
                    ...
                    <x-ui.card as="a" href="{{ route('user-management') }}" variant="interactive" padding="none" class="flex h-full items-start gap-4 p-5 group">
                        ...
                        <div>
                            ...
                            <p class="text-xs text-muted mt-1 leading-relaxed">Sinkronisasi ID akun Keycloak dengan data pegawai SIMPEG.</p>
                        </div>
                    </x-ui.card>

                    <x-ui.card as="a" href="{{ route('audit.index') }}" variant="interactive" padding="none" class="flex h-full items-start gap-4 p-5 group">
                        <div class="flex h-10 w-10 shrink-0 items-center justify-center rounded-lg bg-primary/10 text-primary">
                            <svg class="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
