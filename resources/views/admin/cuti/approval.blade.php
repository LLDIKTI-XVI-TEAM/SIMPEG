<x-layouts.app title="Persetujuan Cuti">
    <div class="space-y-6">
        ...
        <div class="flex flex-col gap-1.5 bg-surface border border-border rounded-lg p-6 shadow-sm">
            ...
            <p class="text-xs text-muted mt-0.5 font-sans">Tinjau dan lakukan keputusan persetujuan/penolakan permohonan cuti dari staf.</p>
                ...
                <table class="w-full">
                    ...
                    <tbody class="divide-y divide-border">
                        ...
                        <tr class="transition-colors hover:bg-soft/30">
                            ...
                            <td class="px-6 py-4 text-sm font-medium text-ink font-sans">{{ $r['jenis'] }}</td>
                            <td class="px-6 py-4 text-sm text-ink font-mono">{{ $r['hari'] }} Hari Kerja<br><span class="text-[10px] text-muted font-sans">{{ \Carbon\Carbon::parse($r['mulai'])->translatedFormat('d M') }} - {{ \Carbon\Carbon::parse($r['selesai'])->translatedFormat('d M Y') }}</span></td>
