        ...
        <div class="rounded-lg border border-border bg-surface shadow-sm overflow-hidden" x-data="{ search: '' }">
            ...
            <div class="overflow-x-auto">
                <table class="w-full">
                    <thead class="bg-soft">
                        <tr>
                            <th class="px-6 py-3.5 text-left text-xs font-semibold uppercase tracking-wider text-muted font-sans w-12 text-center">No</th>
                            <th class="px-6 py-3.5 text-left text-xs font-semibold uppercase tracking-wider text-muted font-sans">Nama / NIP</th>
                            <th class="px-6 py-3.5 text-left text-xs font-semibold uppercase tracking-wider text-muted font-sans">Jenis Event</th>
                            <th class="px-6 py-3.5 text-left text-xs font-semibold uppercase tracking-wider text-muted font-sans">Tanggal Target</th>
                            <th class="px-6 py-3.5 text-left text-xs font-semibold uppercase tracking-wider text-muted font-sans">Sisa Hari</th>
                            <th class="px-6 py-3.5 text-left text-xs font-semibold uppercase tracking-wider text-muted font-sans">Status Eligibility</th>
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-border">
                        @forelse($alerts as $index => $alert)
                            ...
                            @endphp
                            <tr 
                                class="transition-colors {{ $rowColorClass }}"
                                x-show="search === '' || '{{ strtolower($alert['nama']) }}'.includes(search.toLowerCase()) || '{{ str_replace(' ', '', $alert['nip']) }}'.includes(search.replace(/\s+/g, ''))"
                            >
                                <td class="px-6 py-4 text-center font-medium text-muted text-sm w-12">{{ $index + 1 }}</td>
                                <td class="px-6 py-4 text-sm">
                                    <div class="font-semibold text-ink hover:text-primary transition-colors">
                                        <a href="{{ route('pegawai.show', $alert['pegawai_id']) }}">{{ $alert['nama'] }}</a>
                                    </div>
                                    <div class="text-xs text-muted font-mono mt-0.5">{{ $alert['nip'] }}</div>
                                </td>
                                <td class="px-6 py-4 text-sm">
                                    <span class="inline-flex items-center gap-1.5 rounded-md bg-soft px-2.5 py-1 text-xs font-medium text-ink border border-border">
                                ...
                                <td class="px-6 py-4 text-sm text-ink font-medium">
                                    {{ date('d M Y', strtotime($alert['tanggal_target'])) }}
                                </td>
                                <td class="px-6 py-4 text-sm">
