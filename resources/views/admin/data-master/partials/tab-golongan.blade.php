{{-- Tab golongan: data nyata dari database dengan kebijakan hapus hybrid
     (item terpakai hanya boleh dinonaktifkan, item tak terpakai boleh dihapus). --}}
<div x-show="activeTab === 'golongan'"
    x-data="{ showTambah: {{ $errors->any() && old('tab') === 'golongan' ? 'true' : 'false' }}, editId: null }"
    ...
    <div class="pb-4 flex items-center justify-between">
        <div>
            ...
            <p class="text-[11px] text-muted mt-0.5 font-sans leading-normal">Data referensi golongan kepangkatan PNS.
    ...
    <form method="POST" action="{{ route('data-master.golongan.store') }}" x-show="showTambah" style="display: none;"
        ...
        <x-form.input name="kode" label="Kode" required placeholder="cth: III/a" />
        <x-form.input name="nama" label="Nama Pangkat" required placeholder="cth: Penata Muda" />
        ...
        <x-ui.table>
            <x-ui.table-head>
                <x-ui.table-row>
                    ...
                    <x-ui.table-th>Nama Pangkat</x-ui.table-th>
            ...
            <x-ui.table-body>
                @forelse ($golongan as $item)
                    @php $dipakai = $golonganUsage[$item->id] ?? 0; @endphp
                    <x-ui.table-row>
                        ...
                        <x-ui.table-td align="right" padding="sm">
                            <div class="flex items-center justify-end gap-1.5">
                                ...
                                <form method="POST" action="{{ route('data-master.golongan.toggle', $item) }}">
                                    @csrf
                                    <input type="hidden" name="tab" value="golongan">
                                    <button type="submit"
                                ...
                                @if ($dipakai === 0)
                                    <form method="POST" action="{{ route('data-master.golongan.destroy', $item) }}"
                    ...
                    <tr x-show="editId === '{{ $item->id }}'" style="display: none;">
                        <td colspan="6" class="bg-soft/30 px-4 py-4">
                            <form method="POST" action="{{ route('data-master.golongan.update', $item) }}"
                ...
                @empty
                    <tr>
                        <td colspan="6" class="px-4 py-8 text-center text-sm text-muted">Belum ada data golongan.</td>
                    </tr>
                @endforelse
            </x-ui.table-body>
        </x-ui.table>
