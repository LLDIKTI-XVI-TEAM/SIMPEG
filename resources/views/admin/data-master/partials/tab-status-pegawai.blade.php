{{-- Tab status pegawai: data nyata dari database. Baris data sistem (kode
     AKTIF/PENSIUN dan baris default) dikunci: tanpa tombol nonaktif/hapus,
     kode dan nama tidak dapat diubah — backend tetap memvalidasi ulang. --}}
<div x-show="activeTab === 'status_pegawai'"
    x-data="{ showTambah: {{ $errors->any() && old('tab') === 'status_pegawai' ? 'true' : 'false' }}, editId: null }"
    class="rounded-lg bg-surface p-6 shadow-sm space-y-6" style="display: none;">
    <div class="pb-4 flex items-center justify-between">
        <div>
            <h2 class="text-2xl font-bold text-primary font-sans leading-tight">Status Pegawai</h2>
            <p class="text-[11px] text-muted mt-0.5 font-sans leading-normal">Data referensi status kepegawaian.
                Baris bertanda "Data sistem" dipakai langsung oleh EWS dan proses import.</p>
        </div>
        <button type="button" @click="showTambah = !showTambah"
            class="inline-flex items-center justify-center rounded-lg bg-primary px-4 py-2 text-sm font-semibold text-white shadow-sm transition-colors hover:opacity-90">
            Tambah
        </button>
    </div>

    <form method="POST" action="{{ route('data-master.status-pegawai.store') }}" x-show="showTambah" style="display: none;"
        class="grid gap-3 sm:grid-cols-5 items-end rounded-lg border border-border bg-soft/30 p-4">
        @csrf
        <input type="hidden" name="tab" value="status_pegawai">
        <x-form.input name="kode" label="Kode" required placeholder="cth: TUGAS_KHUSUS" />
        <x-form.input name="nama" label="Nama Status" required placeholder="cth: Tugas Khusus" />
        <x-form.input name="kelompok" label="Kelompok" required placeholder="cth: Nonaktif" />
        <x-form.input name="keterangan" label="Keterangan" placeholder="Opsional" />
        <button type="submit"
            class="inline-flex items-center justify-center rounded-lg bg-primary px-4 py-2 text-sm font-semibold text-white shadow-sm transition-colors hover:opacity-90">
            Simpan
        </button>
    </form>

    <div class="rounded-lg overflow-x-auto">
        <x-ui.table>
            <x-ui.table-head>
                <x-ui.table-row>
                    <x-ui.table-th>Kode</x-ui.table-th>
                    <x-ui.table-th>Nama Status</x-ui.table-th>
                    <x-ui.table-th>Kelompok</x-ui.table-th>
                    <x-ui.table-th align="center">Status</x-ui.table-th>
                    <x-ui.table-th align="center">Pemakaian</x-ui.table-th>
                    <x-ui.table-th align="right">Aksi</x-ui.table-th>
                </x-ui.table-row>
            </x-ui.table-head>
            <x-ui.table-body>
                @forelse ($statusPegawai as $item)
                    @php
                        $dipakai = $statusPegawaiUsage[$item->id] ?? 0;
                        $proteksi = $statusPegawaiProtection[$item->id] ?? null;
                    @endphp
                    <x-ui.table-row>
                        <x-ui.table-td padding="sm" class="text-sm font-medium">
                            <span class="inline-flex items-center gap-1.5">
                                {{ $item->kode }}
                                @if ($proteksi !== null)
                                    <span title="{{ $proteksi }}"
                                        class="inline-flex items-center rounded-full bg-info/10 px-2 py-0.5 text-[10px] font-semibold text-info">Data sistem</span>
                                @endif
                                @if ($item->is_default)
                                    <span title="Status default pegawai baru dan hasil import"
                                        class="inline-flex items-center rounded-full bg-primary/10 px-2 py-0.5 text-[10px] font-semibold text-primary">Default</span>
                                @endif
                            </span>
                        </x-ui.table-td>
                        <x-ui.table-td padding="sm" class="text-sm text-muted">{{ $item->nama }}</x-ui.table-td>
                        <x-ui.table-td padding="sm" class="text-sm text-muted">{{ $item->kelompok }}</x-ui.table-td>
                        <x-ui.table-td align="center" padding="sm">
                            @if ($item->is_active)
                                <span class="inline-flex items-center rounded-full bg-success/10 px-2 py-0.5 text-[11px] font-semibold text-success">Aktif</span>
                            @else
                                <span class="inline-flex items-center rounded-full bg-danger/10 px-2 py-0.5 text-[11px] font-semibold text-danger">Nonaktif</span>
                            @endif
                        </x-ui.table-td>
                        <x-ui.table-td align="center" padding="sm" class="text-sm text-muted">{{ $dipakai }} pemakai</x-ui.table-td>
                        <x-ui.table-td align="right" padding="sm">
                            <div class="flex items-center justify-end gap-1.5">
                                <button type="button" @click="editId = editId === '{{ $item->id }}' ? null : '{{ $item->id }}'"
                                    class="rounded-lg border border-border bg-surface px-2.5 py-1.5 text-xs font-semibold text-primary transition-colors hover:bg-soft">Edit</button>
                                @if ($proteksi === null)
                                    <form method="POST" action="{{ route('data-master.status-pegawai.toggle', $item) }}">
                                        @csrf
                                        <input type="hidden" name="tab" value="status_pegawai">
                                        <button type="submit"
                                            class="rounded-lg border border-border bg-surface px-2.5 py-1.5 text-xs font-semibold {{ $item->is_active ? 'text-warning' : 'text-success' }} transition-colors hover:bg-soft">
                                            {{ $item->is_active ? 'Nonaktifkan' : 'Aktifkan' }}
                                        </button>
                                    </form>
                                    @if ($dipakai === 0)
                                        <form method="POST" action="{{ route('data-master.status-pegawai.destroy', $item) }}"
                                            onsubmit="return confirm('Hapus item ini secara permanen? Tindakan tercatat di audit log.')">
                                            @csrf
                                            <input type="hidden" name="tab" value="status_pegawai">
                                            <button type="submit"
                                                class="rounded-lg border border-danger/30 bg-surface px-2.5 py-1.5 text-xs font-semibold text-danger transition-colors hover:bg-danger/10">Hapus</button>
                                        </form>
                                    @endif
                                @endif
                            </div>
                        </x-ui.table-td>
                    </x-ui.table-row>
                    <tr x-show="editId === '{{ $item->id }}'" style="display: none;">
                        <td colspan="6" class="bg-soft/30 px-4 py-4">
                            <form method="POST" action="{{ route('data-master.status-pegawai.update', $item) }}"
                                class="grid gap-3 sm:grid-cols-5 items-end">
                                @csrf
                                <input type="hidden" name="tab" value="status_pegawai">
                                <div class="space-y-1">
                                    <label class="text-[10px] font-bold text-muted uppercase tracking-wide font-sans">Kode</label>
                                    <input name="kode" value="{{ $item->kode }}" required
                                        @if ($proteksi !== null) readonly @endif
                                        class="w-full rounded-xl border border-border px-3 py-2 text-xs text-ink shadow-sm focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary font-sans {{ $proteksi !== null ? 'bg-soft cursor-not-allowed opacity-70' : 'bg-surface' }}">
                                </div>
                                <div class="space-y-1">
                                    <label class="text-[10px] font-bold text-muted uppercase tracking-wide font-sans">Nama Status</label>
                                    <input name="nama" value="{{ $item->nama }}" required
                                        @if ($proteksi !== null) readonly @endif
                                        class="w-full rounded-xl border border-border px-3 py-2 text-xs text-ink shadow-sm focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary font-sans {{ $proteksi !== null ? 'bg-soft cursor-not-allowed opacity-70' : 'bg-surface' }}">
                                </div>
                                <div class="space-y-1">
                                    <label class="text-[10px] font-bold text-muted uppercase tracking-wide font-sans">Kelompok</label>
                                    <input name="kelompok" value="{{ $item->kelompok }}" required
                                        class="w-full rounded-xl border border-border bg-surface px-3 py-2 text-xs text-ink shadow-sm focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary font-sans">
                                </div>
                                <div class="space-y-1">
                                    <label class="text-[10px] font-bold text-muted uppercase tracking-wide font-sans">Keterangan</label>
                                    <input name="keterangan" value="{{ $item->keterangan }}"
                                        class="w-full rounded-xl border border-border bg-surface px-3 py-2 text-xs text-ink shadow-sm focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary font-sans">
                                </div>
                                <button type="submit"
                                    class="inline-flex items-center justify-center rounded-lg bg-primary px-4 py-2 text-sm font-semibold text-white shadow-sm transition-colors hover:opacity-90">
                                    Perbarui
                                </button>
                            </form>
                            @if ($proteksi !== null)
                                <p class="mt-2 text-[11px] text-muted font-sans">Kode dan nama status ini dikunci karena {{ $proteksi }}.</p>
                            @endif
                        </td>
                    </tr>
                @empty
                    <tr>
                        <td colspan="6" class="px-4 py-8 text-center text-sm text-muted">Belum ada data status pegawai.</td>
                    </tr>
                @endforelse
            </x-ui.table-body>
        </x-ui.table>
    </div>
</div>
