...
<div x-show="activeTab === 'jenjang_pendidikan'"
    ...
    <div class="border-b border-border pb-4 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            <h2 class="text-xl font-semibold text-ink font-sans leading-tight">Jenjang Pendidikan</h2>
            <p class="mt-0.5 max-w-2xl text-xs text-muted font-sans leading-normal">Data referensi jenjang pendidikan.
                    ...
                    <x-ui.table-row>
                        ...
                        <x-ui.table-td padding="sm" class="whitespace-nowrap">
                            <div class="flex items-center justify-start gap-1.5">
                                ...
                                <form method="POST" action="{{ route('data-master.jenjang-pendidikan.toggle', $item) }}" class="inline">
                                    @csrf
                                    <input type="hidden" name="tab" value="jenjang_pendidikan">
                                    <x-ui.button
                                        type="submit"
                                ...
                                @if ($dipakai === 0)
                                    <form method="POST" action="{{ route('data-master.jenjang-pendidikan.destroy', $item) }}" class="inline"
                    ...
                    <tr x-show="editId === '{{ $item->id }}'" style="display: none;">
                        <td colspan="5" class="bg-soft/30 px-4 py-4">
                            <form method="POST" action="{{ route('data-master.jenjang-pendidikan.update', $item) }}"
                                ...
                                <input type="hidden" name="tab" value="jenjang_pendidikan">
                                <div class="space-y-1">
                                    <label class="text-xs font-bold text-muted uppercase tracking-wide font-sans">Nama Jenjang</label>
