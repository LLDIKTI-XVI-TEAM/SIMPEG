...
@if($familyMode === 'alpine')
    <td class="px-4 py-3">
        ...
        <p class="text-xs text-muted">NIK. <span x-text="fam.nik || '-'"></span></p>
    </td>
    <td class="px-4 py-3">
    ...
    <td class="px-4 py-3">
        ...
        <p class="text-xs text-muted" x-text="formatDate(fam.tanggal_lahir)"></p>
    </td>
...
@else
    <td class="px-4 py-3">
        ...
        <p class="text-xs text-muted">NIK. @include('pegawai.partials.detail.sensitive-value')</p>
    </td>
    <td class="px-4 py-3">
        <p class="font-sans">{{ $family->hubungan ?: '-' }}</p>
        <p class="text-xs text-muted font-sans">{{ match ($family->jenis_kelamin) { 'L' => 'Laki-laki', 'P' => 'Perempuan', default => '-' } }}</p>
    </td>
    <td class="px-4 py-3">
        <p class="font-sans">{{ $family->tempat_lahir ?: '-' }}</p>
        <p class="text-xs text-muted">@include('pegawai.partials.detail.date', ['value' => $family->tanggal_lahir])</p>
    </td>
    <td class="px-4 py-3 font-sans">{{ $family->pekerjaan ?: '-' }}</td>
    <td class="px-4 py-3">
        <span class="inline-flex items-center gap-1 text-xs font-bold {{ $family->status_tunjangan ? 'text-success' : 'text-muted' }}">
