                    ...
                    >
                        ...
                        @php
                            ...
                            if (auth()->check() && auth()->user()->employee) {
                                $unreadCount = auth()->user()->employee->notifications()->unread()->count();
                                $unreadNotifications = auth()->user()->employee->notifications()->unread()->latest()->take(5)->get();
                            }
                        @endphp
                        @if($unreadCount > 0)
                            <span class="absolute -right-1 -top-1 flex h-4 w-4 items-center justify-center rounded-full bg-danger text-[10px] font-bold text-white">{{ $unreadCount > 9 ? '9+' : $unreadCount }}</span>
                        @endif
                    </button>
                        ...
                        <div class="divide-y divide-border">
                            @forelse($unreadNotifications as $notif)
                                @php
                                    ...
                                    $subText = $notif->data['label'] ?? null;
                                @endphp
                                <div class="flex items-start gap-3 px-4 py-3 transition-colors hover:bg-soft">
                                    <span class="mt-1.5 h-2 w-2 shrink-0 rounded-full bg-{{ $color }}"></span>
                                    <div>
                                        <p class="text-sm font-medium text-ink">{{ $notif->title }}</p>
                                        <p class="mt-0.5 text-xs text-muted font-sans">{{ $notif->body }}</p>
                                        @if($subText)
                                            <span class="mt-1 inline-block text-[10px] font-semibold text-{{ $color }}">{{ $subText }}</span>
                                        @endif
                                    </div>
                                </div>
                            @empty
                                <div class="px-4 py-6 text-center">
                                    <p class="text-sm text-muted">Belum ada notifikasi baru.</p>
                                </div>
                            @endforelse
                        </div>
                    ...
                    >
                        ...
                        <div class="p-1.5 space-y-0.5">
                            ...
                            @if($activeRole === 'super_admin')
                                <a href="{{ route('pengaturan') }}" id="settings-link" class="flex items-center gap-2.5 rounded-lg px-4 py-2 text-sm text-ink transition-colors hover:bg-soft font-sans font-medium">
