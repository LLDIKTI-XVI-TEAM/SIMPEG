...
<nav aria-label="Navigasi halaman" class="w-full max-w-full overflow-x-auto scrollbar-hide sm:w-auto" x-cloak x-show="{{ $total }} > 1">
    <div class="flex w-max min-w-full items-center justify-start gap-1.5 py-1 sm:min-w-0 sm:justify-end">
        {{-- Prev --}}
        <button type="button"
            @click="{{ $prevAction }}"
            :disabled="{{ $current }} === 1"
            :class="{{ $current }} === 1 ? 'opacity-50 cursor-not-allowed text-muted' : 'hover:bg-soft hover:text-ink text-ink cursor-pointer'"
            class="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg border border-border bg-surface shadow-sm transition-colors focus:outline-none focus:ring-2 focus:ring-primary/20">
        ...
        <template x-for="page in {{ $total }}" :key="page">
            ...
            {{-- Page Number --}}
                <button type="button"
                    ...
                    class="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg border text-sm font-semibold transition-colors focus:outline-none focus:ring-2 focus:ring-primary/20 font-sans"
                    x-text="page">
                </button>
