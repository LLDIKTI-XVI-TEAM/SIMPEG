...
Route::middleware($employeeGroupMiddleware)
    ...
    ->group(function () use ($adminEmployeeMutationMiddleware, $adminEmployeeReadMiddleware, $disableEmployeeApiAuth): void {
        ...
        // Mutasi dokumen pegawai dikonsolidasikan ke tab Dokumen & SK pada halaman detail;
        // arsip terpusat hanya baca-saja. Hapus/check-impact tetap eksklusif Super Admin.
        Route::post('/{employee}/berkas-sk', [EmployeeDocumentController::class, 'storeBerkasSk'])
            ->middleware($adminEmployeeMutationMiddleware('employees.update'))
            ->whereUuid('employee')
            ->name('berkas-sk.store');
        Route::post('/{employee}/dokumen', [EmployeeDocumentController::class, 'storeDokumen'])
            ->middleware($adminEmployeeMutationMiddleware('employees.update'))
            ->whereUuid('employee')
            ->name('dokumen.store');
        Route::post('/{employee}/dokumen/{document}', [EmployeeDocumentController::class, 'updateDokumen'])
            ->middleware($adminEmployeeMutationMiddleware('employees.update'))
        ...
        Route::delete('/{employee}/dokumen/{document}', [EmployeeDocumentController::class, 'destroy'])
            ->middleware($disableEmployeeApiAuth ? [] : ['role:super_admin'])
            ->whereUuid(['employee', 'document'])
