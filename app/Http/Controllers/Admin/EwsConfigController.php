...
class EwsConfigController extends Controller
{
    ...
    public function index()
    {
        ...
        $dbLogs = \App\Models\AuditLog::where('auditable_type', 'EwsConfig')
    ...
    public function update(Request $request)
    {
        ...
        if ($p90 <= $p60) $thresholdErrors['pangkat_h60'][] = 'Pangkat Tahap 1 harus lebih besar dari Tahap 2.';
        if ($p60 <= $p30) $thresholdErrors['pangkat_h30'][] = 'Pangkat Tahap 2 harus lebih besar dari Tahap 3.';

        $k60 = (int) $request->input('kgb_h60');
        $k30 = (int) $request->input('kgb_h30');
        $k14 = (int) $request->input('kgb_h14');
        if ($k60 <= $k30) $thresholdErrors['kgb_h30'][] = 'KGB Tahap 1 harus lebih besar dari Tahap 2.';
        if ($k30 <= $k14) $thresholdErrors['kgb_h14'][] = 'KGB Tahap 2 harus lebih besar dari Tahap 3.';

        $py1 = (int) $request->input('pensiun_y1');
        $pm6 = (int) $request->input('pensiun_m6');
        $pm3 = (int) $request->input('pensiun_m3');
        if ($py1 <= $pm6) $thresholdErrors['pensiun_m6'][] = 'Pensiun Tahap 1 harus lebih besar dari Tahap 2.';
        if ($pm6 <= $pm3) $thresholdErrors['pensiun_m3'][] = 'Pensiun Tahap 2 harus lebih besar dari Tahap 3.';

        $pp6 = (int) $request->input('pppk_m6');
        $pp3 = (int) $request->input('pppk_m3');
        $pp1 = (int) $request->input('pppk_m1');
        if ($pp6 <= $pp3) $thresholdErrors['pppk_m3'][] = 'PPPK Tahap 1 harus lebih besar dari Tahap 2.';
        if ($pp3 <= $pp1) $thresholdErrors['pppk_m1'][] = 'PPPK Tahap 2 harus lebih besar dari Tahap 3.';
